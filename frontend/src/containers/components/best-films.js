export function BestFilms() {
  const html = `
    <section class="w-full mb-12">
      ...
      <div id="best-films-toggle-container" class="lg:hidden flex justify-center mt-6 hidden">
        <button id="best-films-toggle-btn" class="bg-red-600 text-white text-2xl font-normal px-16 py-4 rounded-full hover:bg-red-700 transition">
          Voir plus
        </button>
      </div>
    </section>
  ...
  requestAnimationFrame(() => {
    fetch(
      ...
      .then((data) => {
        ...
        // Afficher le bouton toggle sur mobile et tablette
        const toggleContainer = document.getElementById(
          "best-films-toggle-container",
        );
        if (toggleContainer) {
        ...
        const toggleBtn = document.getElementById("best-films-toggle-btn");
        ...
        if (toggleBtn) {
          toggleBtn.addEventListener("click", () => {
            ...
            if (!isExpanded) {
              ...
              // Masquer les films selon le breakpoint
