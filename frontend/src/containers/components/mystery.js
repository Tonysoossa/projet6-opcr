export function MysteryFilms() {
  ...
  requestAnimationFrame(() => {
    fetch(
      "http://localhost:8000/api/v1/titles/?genre=Mystery&sort_by=-imdb_score&page_size=50",
    )
      .then((res) => res.json())
      .then((data) => {
        const grid = document.getElementById("mystery-films-grid");
        if (!grid) return;
